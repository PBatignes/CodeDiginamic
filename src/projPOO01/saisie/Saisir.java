...
public class Saisir {
	...
	public static void SaisirInterimaire() {
		...
		boolean erreurdm = true;

		for (int i = 0; i < 2; i++) {
			System.out.println("Saisir le nom de l'intérimère");
			nom = Menus.sc.next();
			System.out.println("Saisir le prenom de l'intérimère");
			prenom = Menus.sc.next();
			System.out.println("Saisir l'adresse de l'intérimère");
			adresse = Menus.sc.next();
			System.out.println("Saisir la ville de l'intérimère");
			ville = Menus.sc.next();
			erreurcp = true;
			while (erreurcp) {
				try {
					System.out.println("Saisir le codepostal de l'intérimère");
					codepostal = Menus.sc.next();
					Salarie.CtrlCodePostal(codepostal);
					erreurcp = false;
				} catch (ExceptionSaisieCodePostal e) {
			...
			while (erreurns) {
				try {
					System.out.println("Saisir le numero de sécurité sociale de l'intérimère");
					secu = Menus.sc.next();
					Salarie.CtrlSaisiNumeroSecu(secu);
					erreurns = false;
				} catch (ExceptionSaisiNumeroSecu e) {
			...
			salaire = 0;
			System.out.println("Saisir le salaire de l'intérimère");
			while (Menus.sc.hasNext() && salaire == 0) {
			...
			while (erreurca) {
				...
				try {
					...
					System.out.println("Saisir le numero de contrat de l'agence de l'intérimère");
					...
					System.out.println(e.getMessage() + " " + contratAgence.length());
				}
			...
			while (erreurdm) {
				...
				try {
					...
					System.out.println("Saisir la durée du contrat de l'intérimère (en mois)");
					dureeMois = Menus.sc.nextInt();
					Interimaire.ctrlDureeMois(dureeMois);
					erreurns = false;

				} catch (ExceptionInt e) {
					// TODO: handle exception
					System.out.println(e.getMessage() + " " + dureeMois);
				}

			}

			Interimaire interimere = new Interimaire(nom, prenom, adresse, ville, codepostal, secu, salaire,
					contratAgence, dureeMois);

			il.add(interimere);
		}

		listInterimere = new ArrayList<Personne>(il);
		if (Menus.choixmenu != 1) {
			Menus.Menu();
		}
		return;
